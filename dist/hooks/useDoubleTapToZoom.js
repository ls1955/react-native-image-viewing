 ...
 */
function useDoubleTapToZoom(scrollViewRef, scaled, screen, imageDimensions) {
    const handleDoubleTap = useCallback((event) => {
        ...
        const getZoomRectAfterDoubleTap = (touchX, touchY) => {
            if (!imageDimensions) {
                return {
                    x: 0,
                    y: 0,
            ...
            const zoom = Math.max(imageAspect / screenAspect, screenAspect / imageAspect, MIN_ZOOM);
